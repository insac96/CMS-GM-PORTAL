...
export default defineEventHandler(async (event) => {
  try {
    ...
    const { size, current, sort, game: gameID, type } = await readBody(event)
    ...
    const match : any = { game: new Types.ObjectId(game._id), type: type }
    ...
    const total = await DB.GamePrivateEvent.count(match)
