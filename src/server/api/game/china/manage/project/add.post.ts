...
export default defineEventHandler(async (event) => {
  try {
    ...
    const nameCheck = await DB.GameChina.findOne({ key: key }).select('_id')
    if(!!nameCheck) throw 'Tên trò chơi đã tồn tại'

    const shortNameCheck = await DB.GameChina.findOne({ short_name: short_name }).select('_id')
    if(!!shortNameCheck) throw 'Tên ngắn trò chơi đã tồn tại'

    body.key = key
    await DB.GameChina.create(body)
    logAdmin(event, `Thêm trò chơi China <b>${name}</b>`)
