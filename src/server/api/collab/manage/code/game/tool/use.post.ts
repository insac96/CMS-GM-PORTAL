...
import type { IAuth, IDBCollab, IDBGameTool } from "~~/types"
...
export default defineEventHandler(async (event) => {
  try {
    ...
    if(type == 'toggle-single'){
      ...
      const game = await DB.GameTool.findOne({ _id: gameID }).select('collab') as IDBGameTool
      const check = game.collab.use.find((item: Types.ObjectId) => item.toString() == collab._id.toString())

      if(!check) await DB.GameTool.updateOne({ _id: game._id }, { $push: { 'collab.use': collab._id } })
      else await DB.GameTool.updateOne({ _id: game._id }, { $pull: { 'collab.use': collab._id } })
    }

    else if(type == 'add-all') {
      await DB.GameTool.updateMany({ }, { $push: { 'collab.use': collab._id } })
    }

    else if(type == 'del-all') {
      await DB.GameTool.updateMany({ }, { $pull: { 'collab.use': collab._id } })
    }
