...
import type { IAuth, IDBCollab, IDBGamePrivate } from "~~/types"
...
export default defineEventHandler(async (event) => {
  try {
    ...
    if(type == 'toggle-single'){
      ...
      const game = await DB.GamePrivate.findOne({ _id: gameID }).select('collab') as IDBGamePrivate
      const check = game.collab.use.find((item: Types.ObjectId) => item.toString() == collab._id.toString())

      if(!check) await DB.GamePrivate.updateOne({ _id: game._id }, { $push: { 'collab.use': collab._id } })
      else await DB.GamePrivate.updateOne({ _id: game._id }, { $pull: { 'collab.use': collab._id } })
    }

    else if(type == 'add-all') {
      await DB.GamePrivate.updateMany({ }, { $push: { 'collab.use': collab._id } })
    }

    else if(type == 'del-all') {
      await DB.GamePrivate.updateMany({ }, { $pull: { 'collab.use': collab._id } })
    }
