  ...
  try {
    ...
    if(type != 'total'){
      ...
      payment = await DB.Payment.aggregate([
        ...
        { $match: { 'user.reg.collab': adsCollab._id } },
        {
          $project: {
            createdAt: 1,
            timeformat: {
      ...
      signup = await DB.User.aggregate([
        { $match: { 'reg.collab': adsCollab._id } },
        {
          $project: {
            createdAt: 1,
            timeformat: {
    ...
    if(type == 'total') {
      payment = await DB.Payment.aggregate([
        ...
        { $unwind: { path: "$user", preserveNullAndEmptyArrays: true }},
        { $match: { 'status': 1, 'user.reg.collab': adsCollab._id } },
        {
          $group: {
            _id: null,
            money: { $sum: '$money' },
      ...
      const users = await DB.User.count({ 'reg.collab': adsCollab._id })
      signin = [{ count: users }]
      signup = [{ count: users }]
    }
