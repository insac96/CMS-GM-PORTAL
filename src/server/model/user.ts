...
export const DBUserLevel = (mongoose : Mongoose) => {
  ...
  const autoCreate = async () => {
    ...
    // Default
    if(lv1 == 0) await model.create({ number: 1, exp: 0, stone: 1, bonus: { payment: 0 }, limit: { chat: 5 }, discount: { shop: 0 } })
    if(lv2 == 0) await model.create({ number: 2, exp: 50000, stone: 2, bonus: { payment: 1 }, limit: { chat: 10 }, discount: { shop: 1 } })
    if(lv3 == 0) await model.create({ number: 3, exp: 500000, stone: 2, bonus: { payment: 1 }, limit: { chat: 15 }, discount: { shop: 1 } })
    if(lv4 == 0) await model.create({ number: 4, exp: 1000000, stone: 2, bonus: { payment: 2 }, limit: { chat: 20 }, discount: { shop: 2 } })
    if(lv5 == 0) await model.create({ number: 5, exp: 3000000, stone: 3, bonus: { payment: 3 }, limit: { chat: 25 }, discount: { shop: 3 } })
    if(lv6 == 0) await model.create({ number: 6, exp: 5000000, stone: 3, bonus: { payment: 4 }, limit: { chat: 30 }, discount: { shop: 4 } })
    if(lv7 == 0) await model.create({ number: 7, exp: 10000000, stone: 3, bonus: { payment: 5 }, limit: { chat: 35 }, discount: { shop: 5 } })
    if(lv8 == 0) await model.create({ number: 8, exp: 20000000, stone: 4, bonus: { payment: 10 }, limit: { chat: 40 }, discount: { shop: 10 } })
    if(lv9 == 0) await model.create({ number: 9, exp: 50000000, stone: 4, bonus: { payment: 12 }, limit: { chat: 50 }, discount: { shop: 12 } })
    if(lv10 == 0) await model.create({ number: 10, exp: 100000000, stone: 5, bonus: { payment: 20 }, limit: { chat: 1000 }, discount: { shop: 20 } })
  }
