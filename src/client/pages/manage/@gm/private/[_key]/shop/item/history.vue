<template>
  <UiContent title="Lịch Sử Mua Vật Phẩm" sub="Giao dịch mua vật phẩm của người chơi" no-dot>
    <template #more>
			<UButton 
				icon="i-bx-list-ul" 
				size="xs" 
				class="ml-auto"
				:to="`/manage/@gm/private/${game.key}/shop/item`"
			>
				Danh sách
			</UButton>
		</template>
...
const getList = async () => {
  try {
    ...
    const data = await useAPI('game/private/manage/shop/item/history', JSON.parse(JSON.stringify(page.value)))
