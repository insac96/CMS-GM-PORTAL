...
const send = async () => {
  try {
    ...
    await useAPI('game/private/manage/project/vps/mail', JSON.parse(JSON.stringify(stateSend.value)))
...
const play = async (row) => {
  try {
    ...
    const data = await useAPI('game/private/manage/project/vps/play', {
      account: row.account,
