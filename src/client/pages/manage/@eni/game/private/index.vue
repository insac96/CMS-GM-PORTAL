<template>
  <UiContent title="Game Private" sub="Quản lý các trò chơi Private">
...
const actions = (row) => [
  [{
    label: 'Quản lý',
    icon: 'i-bx-server',
    click: () => useTo().openNewTab(`/manage/@gm/private/${row.key}`)
  }],[{
    label: 'Sửa thông tin',
    icon: 'i-bx-pencil',
    click: () => {
...
const getList = async () => {
  try {
    ...
    const data = await useAPI('game/private/manage/project/list', JSON.parse(JSON.stringify(page.value)))
...
const addAction = async () => {
  try {
    ...
    await useAPI('game/private/manage/project/add', JSON.parse(JSON.stringify(stateAdd.value)))
...
const delAction = async (_id) => {
  try {
    ...
    await useAPI('game/private/manage/project/del', { _id })
