<template>
  <div v-if="list.length > 0">
    <UiText align="center" class="font-bold tracking-tight text-gray-900 dark:text-gray-100 sm:text-3xl text-xl mb-6">
      Game <span class="text-primary">Tool</span> Nổi Bật
    </UiText>
    
    <DataGameList :loading="loading" :list="list" os="tool" max="4" />

    <UiFlex justify="center" class="mt-4">
      <UButton size="sm" color="gray" @click="navigateTo('/game/tool')">Xem Thêm</UButton>
    </UiFlex>
  </div>
</template>
...
const getList = async () => {
  try {
    loading.value = true
    const data = await useAPI('game/tool/public/list/latest')
